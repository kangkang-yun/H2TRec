...
def collate_fn(batch_data):
    ...
    """
    uids, iids, labels = [], [], []
    u_items, u_users, u_users_items, i_users, sf_users, sf_users_items, i_friends, if_items_users, u_fans, u_fans_items = [], [], [], [], [], [], [], [], [], []
    u_items_len, u_users_len, i_users_len, sf_users_len, sf_items_len, i_friends_len, if_users_len, u_fans_len = [], [], [], [], [], [], [], []
    neg_transh_list, pos_transh_list = [], []
    neg_len = []
    count = 0
    for data, u_items_u, u_users_u, u_users_items_u, i_users_i, u_sf_users, u_sf_users_items, i_items_friends, i_items_users, u_fans_u, u_fans_items_u, neg_tr_list in batch_data:
        ...
        # user_fans and user_fans_items
        if len(u_fans_u) < truncate_len:
            tmp_fans = [item for item in u_fans_u]
            tmp_fans.append(uid)
            u_fans.append(tmp_fans)
            u_f_items = []
            for ufi in u_fans_items_u:
                if len(ufi) < truncate_len:
                    u_f_items.append(ufi)
                else:
                    u_f_items.append(random.sample(ufi, truncate_len))
            u_f_items.append(u_items[-1])
            u_fans_items.append(u_f_items)

        else:
            sample_index = random.sample(list(range(len(u_fans_u))), truncate_len-1)
            tmp_fans = [u_fans_u[si] for si in sample_index]
            tmp_fans.append(uid)
            u_fans.append(tmp_fans)

            u_fans_items_u_tr = [u_fans_items_u[si] for si in sample_index]
            u_f_items = []
            for ufi in u_fans_items_u_tr:
                if len(ufi) < truncate_len:
                    u_f_items.append(ufi)
                else:
                    u_f_items.append(random.sample(ufi, truncate_len))
        ...
        u_fans_len.append(min(len(u_fans_u)+1, truncate_len))
    ...
    u_fans_pad = torch.zeros([batch_size, u_fans_maxlen], dtype=torch.long)
    for i, uf in enumerate(u_fans):
        u_fans_pad[i, :len(uf)] = torch.LongTensor(uf)
    ...
    u_fans_item_pad = torch.zeros([batch_size, u_fans_maxlen, u_items_maxlen, 2], dtype=torch.long)
    for i, uf_items in enumerate(u_fans_items):
        for j, fi in enumerate(uf_items):
            u_fans_item_pad[i, j, :len(fi), :] = torch.LongTensor(fi)
    ...
    return torch.LongTensor(uids), torch.LongTensor(iids), torch.FloatTensor(labels), \
            u_item_pad, u_user_pad, u_user_item_pad, i_user_pad, sf_user_pad, sf_user_item_pad, i_friends_pad, if_items_users_pad, u_fans_pad, u_fans_item_pad, pos_pad, neg_pad
